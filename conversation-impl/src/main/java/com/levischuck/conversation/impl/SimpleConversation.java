...
public class SimpleConversation<C, M, D, S> implements Conversation<C, M, D, S> {
    ...
    @Override
    public StepResult<C, M, D, S> converse(C context, M message, D dialogDescriptor, S stepDescriptor) {
        ...
        StepResult<C, M, D, S> stepResult = step.converse(context, message);
        if (stepResult == null) {
            throw new NullPointerException("stepResult from step converse is null");
        }
        return stepResult;
    }
}
